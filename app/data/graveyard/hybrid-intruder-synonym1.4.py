...
def get_jargon_to_avoid(text, freq_ranks=None):
    ...
    Features:
      - LLM-guided detection
      - strong post-filtering (deterministic safety)
      - adaptive max size based on text complexity
      - ranking by rarity + length
    """

    system_prompt = (
        "You are identifying words that should NOT be replaced in a lexical substitution task.\n\n"
        "Return a list of words that MUST remain unchanged because they are domain-specific "
        "technical terms that would sound unnatural if replaced.\n\n"
        "Examples: wearable, robotics, blockchain, enzyme, convolution.\n\n"
        "Only include words that:\n"
        "- refer to specific technical concepts\n"
        "- have no natural everyday synonym\n\n"
        "Output rules:\n"
        "- lowercase only\n"
        "- one word per line\n"
        "- no explanation\n"
    ...
    # ----------------------------
    try:
        raw = llm_chat(system_prompt, text, temperature=0.2, max_tokens=100)
    except Exception as e:
    ...
    for line in raw.splitlines():
        ...
        # hard guardrail (prevents classic false positives)
        if w in {"analysis", "system", "method", "process", "result"}:
            continue
    ...
    # estimate complexity based on unique content words
    tokens = tokenize_words_lower(text)
    content_words = [t for t in tokens if t not in STOPWORDS]

    unique_content = len(set(content_words))

    # adaptive cap: ~20% of candidate space, bounded
    adaptive_cap = max(5, min(12, int(unique_content * 0.2)))

    if len(words) > adaptive_cap:
        print(
            f"⚠️ Jargon list too large ({len(words)}) — trimming to {adaptive_cap}"
        )

        # ----------------------------
        # RANKING (rarity + length)
        # ----------------------------
        def rank_key(w):
            freq_rank = freq_ranks.get(w, 10**9) if freq_ranks else 10**9
            return (-freq_rank, -len(w), w)

        words = set(sorted(words, key=rank_key)[:adaptive_cap])

    print(f"🧠 Jargon detected (avoid): {sorted(words)}")
...
def generate_pdf(student_id, name, sentences, replacements, pdf_dir=PDF_DIR):
    ...
    html_parts = [
        ...
        "<b>Incorrect sentence numbers (−1 point). Incorrect word choices (0 points).<br>",
        "</div>",
