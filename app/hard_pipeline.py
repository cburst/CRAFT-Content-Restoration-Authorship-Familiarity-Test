...
def run_hard_pipeline(input_tsv, testname):
    """
    Runs the LLM-generated ("hard") pipeline.
    ...
    timestamp, log_file = setup_log("hard")

    log(f"▶ Starting HARD pipeline: {testname}", log_file)
    ...
    # ---------------------------------------------------------
    # 2. Generate LLM-authored TSV
    # ---------------------------------------------------------
    log("▶ Generating LLM-authored TSV...", log_file)

    run_script("llmtextgenerator.py", log_file, "students.tsv", "llmoutput.tsv")

    llm_output = os.path.join(WORKSPACE, "llmoutput.tsv")

    if not os.path.exists(llm_output):
        raise RuntimeError("❌ llmoutput.tsv not produced")

    os.replace(llm_output, students_path)
    log("✔ Replaced students.tsv with LLM output", log_file)

    # ---------------------------------------------------------
    # 3. Run hybrid generator
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 7. Move final outputs (clean naming)
    # ---------------------------------------------------------
    safe_name = "".join(
